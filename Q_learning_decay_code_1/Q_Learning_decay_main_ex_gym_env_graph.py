...
def updateQ(observation, action, reward, next_observation, alpha, gamma, Q, env):
...
base_path = f"C:\\-----\\-----\\----"
save_path = os.path.join(base_path, "Q_raw_decay_data_1")
        ...
        for alpha in alpha_values:
            for gamma in gamma_values:
                ...
                for episode in range(nbr_episodes):
                    ...
                    while True:
                        action = policy(observation, epsilon, Q, env)
                        next_observation, reward, terminated, truncated, info = env.step(action)
                        next_observation = str(next_observation)
                        updateQ(observation, action, reward, next_observation, alpha, gamma, Q, env)
